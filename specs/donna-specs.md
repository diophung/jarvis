...
Your mission is to autonomously implement Jarvis, a digital executive assistant that helps busy professionals understand what matters most across email, chat, calendar, cloud storage, and uploaded files.
...
Jarvis is a personal digital executive assistant for busy professionals. Jarvis connects to live systems such as email, chat, calendar, S3, Google Drive, OneDrive, and uploaded files. Jarvis learns user preferences over time, understands what is important, identifies what is urgent, estimates what requires the most effort, and presents a clear daily debrief and digest that helps the user prioritize and plan.

Jarvis’s job is not to dump notifications. Jarvis’s job is to reduce executive cognitive load. It should act like a smart chief of staff: identify the few things that matter, explain why they matter, recommend what to do next, and help the user plan the day.
...
At the end of implementation, Jarvis must support a user who can run the app locally, configure an LLM provider, connect or simulate data sources, upload documents, chat with Jarvis, generate a daily debrief, inspect prioritized tasks, understand why each item matters, adjust preferences, configure permissions, approve or deny agentic actions, and review audit logs.

Jarvis must be deployable both locally and in the cloud.
...
Implement the system in vertical slices, not disconnected scaffolding. A user should be able to run Jarvis locally, open the app, configure settings, upload data, see sample connected-source data through mock or real connectors, receive a generated digest, and interact with Jarvis through a ChatGPT-style interface.

# Architecture Requirements

Design Jarvis around these major subsystems:
...
7. Assistant Orchestration Layer — allows Jarvis to answer questions, produce digests, summarize sources, recommend next actions, and explain reasoning in user-readable terms.
8. Permission and Approval Layer — controls what Jarvis can do automatically and what requires user approval.
...
Each item must preserve source metadata, timestamps, ownership, permissions, provenance, and retrieval references so Jarvis can explain where information came from.

# LLM Provider Requirements

Jarvis must support configurable LLM integration with both local inference servers and cloud models.
...
If no cloud or local LLM is configured, Jarvis should still run in demo mode with mock responses and clear UI warnings. The app should not crash because an API key is missing.

Add a Settings tab called Models or AI Providers where users can configure local or cloud model providers. Include a health-check action that verifies whether Jarvis can reach the configured model.
...
For uploaded files, implement drag-and-drop upload, file metadata extraction, text extraction for common formats where feasible, source attribution, chunking, indexing, and inclusion in Jarvis’s search and digest pipeline. Uploaded files should be treated as first-class sources, not as a side feature.
...
Implement feedback loops. The user should be able to mark an item as important, not important, urgent, not urgent, done, deferred, incorrect, or “more like this.” Jarvis should use this feedback to update preferences and future scoring.
...
Implement Jarvis as a chat-first assistant similar to ChatGPT or Claude.

The main screen should have a left sidebar for conversations, daily debriefs, settings, connected sources, uploaded files, and digest history. The central panel should be a conversation with Jarvis. A contextual side panel may show sources, tasks, approvals, or digest details.
...
Jarvis’s responses should be structured, concise, and actionable. The UI should allow Jarvis to cite sources behind its answers. Source citations should open the underlying normalized item or connector reference where available.

When Jarvis is uncertain, it should say so and explain what data is missing.

Add contextual actions next to Jarvis’s recommendations. Examples include mark done, defer, create draft, create task, schedule follow-up, open source, ask why, change priority, ignore similar items, and add preference.
...
Implement an approval queue. When Jarvis wants to perform a gated action, it should create an ApprovalRequest with action type, target source, affected object, risk level, reason, preview of the change, and allow/deny controls.
...
Jarvis should never pretend memory is magic. It should expose what it believes and let the user correct it.
...
Retrieval should preserve source attribution. Jarvis must know whether a fact came from an email, calendar event, chat message, uploaded document, memory, or generated digest.
...
Jarvis must support local deployment and cloud deployment.
...
Implement basic redaction for logs. Use secure defaults. Make it easy to run Jarvis without connecting real accounts by using mock data and uploaded files.
...
The Settings page should be polished and understandable. Permission controls should use plain language. Avoid showing raw capability names unless helpful. For example, show “Allow Jarvis to summarize emails automatically” instead of only “email.read.summary.autoApprove.”
...
The implementation is complete when a user can run Jarvis locally, configure either a local or cloud LLM provider, open the ChatGPT/Claude-style UI, upload files, view mock or connected source data, generate a daily debrief, ask Jarvis prioritization questions in chat, inspect why items were prioritized, adjust preferences, manage memory, configure permissions, approve or deny agent actions, review audit logs, and understand whether their model processing is local or cloud-based.
...
Now implement Jarvis autonomously.