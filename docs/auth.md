...
Everything about signing in to Donna and authorizing Donna to read your data:
...
`DONNA_AUTH_MODE` selects one of two modes (default `local`):
...
| `password` | Email + password login, self-service registration (`DONNA_ALLOW_SIGNUP`, default on), and any configured OAuth login providers. | Anything reachable by other people. |

In `password` mode the first owner account comes from `DONNA_OWNER_EMAIL` /
`DONNA_OWNER_NAME` / `DONNA_OWNER_PASSWORD` on **first boot** (the password is
...
Cookie attributes (`donna_session`):
...
| `HttpOnly` | yes |
| `SameSite` | `Lax` |
| `Secure` | `DONNA_COOKIE_SECURE` (set `true` behind HTTPS) |
| Signed | yes, with `DONNA_SECRET` (`@fastify/cookie`) |
| `Max-Age` | 30 days |
...
This is the most important distinction in Donna's auth design:
**authentication** (who you are) and **data-source authorization** (what data
Donna may read) are related but separate. Signing in with Google does **not**
give Donna access to your Gmail, Drive, or Calendar — those are explicit,
...
Browser                      Donna API                         Provider
   ...
   │ ◄──302 returnTo + donna_session cookie (no provider tokens kept)
```

**Data-source authorization** — an *already signed-in* user grants Donna
least-privilege read access to one Google source; tokens are stored encrypted
and used server-side by the connector layer:

```
Browser (signed in)          Donna API                         Google
...
Separate routes, separate state cookies (`donna_oauth_login` vs
`donna_oauth_source`), separate tables (`auth_accounts` vs `oauth_tokens`).
...
"Continue with …" buttons. Redirect URIs are built from `DONNA_PUBLIC_URL`
...
Redirect URI to register: `<DONNA_PUBLIC_URL>/api/auth/oauth/google/callback`
...
Redirect URI to register: `<DONNA_PUBLIC_URL>/api/auth/oauth/facebook/callback`
...
Redirect URI to register: `<DONNA_PUBLIC_URL>/api/auth/oauth/apple/callback`
...
- **HTTPS only.** Apple does not accept `http://` or `localhost` return URLs,
  and Donna's Apple flow itself requires HTTPS: Apple posts the callback as a
  cross-site `form_post`, so the state cookie must be `SameSite=None`, which
  browsers only accept (and send) as a `Secure` cookie. Set
  `DONNA_COOKIE_SECURE=true` and serve Donna over HTTPS (for local testing,
  use an HTTPS tunnel and point `DONNA_PUBLIC_URL` at it).
- **The client secret is not a string** — it is a short-lived **ES256 JWT**
  Donna mints per token call from your Sign in with Apple key.
...
1. Certificates, Identifiers & Profiles → **Identifiers**: create an App ID,
   ...
   (e.g. `com.example.donna.web`) is your `APPLE_CLIENT_ID`. Configure it
...
```
APPLE_CLIENT_ID=com.example.donna.web      # the Services ID, not the App ID
...
Apple quirks Donna handles: the user's name arrives only on the **first**
...
When an OAuth callback resolves to a Donna user (`resolveOauthLogin` in
...
| Identity unknown, no matching user, signup allowed (`DONNA_ALLOW_SIGNUP`, or local mode) | New user + workspace provisioned, identity linked, signed in. |
...
```
<DONNA_PUBLIC_URL>/api/auth/oauth/google/callback        # login
<DONNA_PUBLIC_URL>/api/sources/oauth/google/callback     # data sources
```
...
`openid email` so Donna can pin the grant to the granting Google account:

| Source | Scope requested | Donna can | Donna cannot |
|---|---|---|---|
...
`invalid_grant` / 400 / 401, or no usable refresh token exists), Donna marks
...
- **Encrypted at rest**: access and refresh tokens in `oauth_tokens` are
  ...
  `v1:<iv>:<tag>:<ciphertext>`) with `DONNA_TOKEN_ENCRYPTION_KEY`, falling
  back to `DONNA_SECRET` when unset.
...
### Encryption keys (`DONNA_TOKEN_ENCRYPTION_KEY` / `DONNA_SECRET`)
...
`refresh_token_encrypted` under the current `DONNA_TOKEN_ENCRYPTION_KEY`
(fallback `DONNA_SECRET`) and `llm_provider_configs.api_key_encrypted` under
the current `DONNA_SECRET`. It prints counts only — never secret material —
and exits non-zero if any row did not decrypt with `DONNA_OLD_KEY` (those
...
Also note: rotating `DONNA_SECRET` invalidates every session **cookie**
...
Donna sends no email, so there is no reset link — the `/forgot-password` page
...
```bash
pnpm --filter @donna/server exec tsx src/scripts/reset-password.ts you@example.com 'a-new-strong-password'
```

Run it where the server's env (`DATABASE_URL` / `DONNA_DATA_DIR`) is visible
— e.g. `docker compose exec donna pnpm --filter @donna/server exec tsx
...
`DONNA_PUBLIC_URL` is the public base URL of the **API**; every redirect URI
is `<DONNA_PUBLIC_URL>` + a fixed path. Unset, it defaults to
`http://localhost:<DONNA_PORT>` (so `http://localhost:3001`).

| Deployment | `DONNA_PUBLIC_URL` | Register at each provider |
|---|---|---|
...
| Cloud / reverse proxy at `https://donna.example.com` | `https://donna.example.com` (required — the server cannot guess its external URL) | `https://donna.example.com/api/auth/oauth/<provider>/callback` and `https://donna.example.com/api/sources/oauth/google/callback` |
...
app (`DONNA_WEB_ORIGIN` in split-origin dev, same-origin otherwise). A
trailing slash on `DONNA_PUBLIC_URL` is stripped. Apple needs an HTTPS URL in
...
| Symptom | Cause / fix |
|---|---|
| Google `Error 400: redirect_uri_mismatch` | The registered URI must match **exactly** (scheme, host, port, path). Set `DONNA_PUBLIC_URL` to the URL in your browser's address bar and re-check the console entry. |
...
| Redirected to `/signin?error=oauth_state_mismatch` (or `?sourceError=`) | The 10-minute state cookie expired on the consent screen, third-party cookies are blocked, or you started the flow on a different host than `DONNA_PUBLIC_URL`. Retry promptly, from the public URL. |
| Apple login always fails state validation over HTTP | Expected: Apple's `form_post` callback needs a `SameSite=None; Secure` state cookie, which browsers drop on plain HTTP. HTTPS + `DONNA_COOKIE_SECURE=true` only. |
